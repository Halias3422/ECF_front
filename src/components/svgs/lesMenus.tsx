import * as React from 'react';

const SvgLesMenus = (props: any) => (
  <svg
    width={48}
    height={48}
    viewBox="0 0 24 24"
    id="Layer_1"
    data-name="Layer 1"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <defs>
      <style>
        {
          '.cls-1{fill:none;stroke:#020202;stroke-miterlimit:10;stroke-width:1.91px}'
        }
      </style>
    </defs>
    <path
      className="cls-1"
      d="M11.04 20.59H1.5V1.5h15.27v7.64M4.36 5.32h1.91M8.18 5.32h5.73M4.36 9.14h1.91M8.18 9.14h5.73M4.36 12.95h1.91M8.18 12.95h5.73"
    />
    <ellipse className="cls-1" cx={17.73} cy={18.68} rx={4.77} ry={3.82} />
    <path
      className="cls-1"
      d="M20.11 11h1.43v1.43a2.39 2.39 0 0 1-2.39 2.39h-1.42v-1.39A2.39 2.39 0 0 1 20.11 11Z"
    />
    <path className="cls-1" d="M14.86 11.05a2.86 2.86 0 0 1 2.87 2.86v.95" />
  </svg>
);

export default SvgLesMenus;
